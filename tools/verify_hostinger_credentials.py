...
def verify_credentials():
    """Alias for unified_verifier import surface."""
    try:
        creds = load_credentials()
    except Exception as e:
    ...
    if not isinstance(creds, dict):
        return {
            "ok": False,
            "error": "credentials_not_dict",
            "raw_type": type(creds).__name__,
        }
    # Prefer structured pass-through; do not open live SSH in --verify default path
    return {
        "ok": True,
        "credentials_loaded": True,
        "keys": sorted(list(creds.keys()))[:20],
    }
