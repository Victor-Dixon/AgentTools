...
def test_maskzero_dry_run_pass():
    if not MASKZERO.is_file() or not (GAB / "runtime/scripts/approved_branch_delete_lib.py").is_file():
...
def test_list_webhooks_never_returns_urls(monkeypatch):
    ...
    assert "webhook_env_keys" not in out  # names that embed secrets must stay out of responses
