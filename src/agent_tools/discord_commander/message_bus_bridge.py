...
def _poll_bus_message(
    *,
    ...
    while time.monotonic() < deadline:
        try:
            last = get_message(message_id=message_id, state_path=paths.state_path)
        except Exception:
            # Enqueue race: message not yet visible in bus_state (RuntimeBusError, not KeyError)
            last = {}
...
def send_via_message_bus(
    *,
    ...
    """Enqueue D2A on unified bus; single delivery owner (processor OR inline, never both)."""
    ...
    try:
        ...
        from dreamvault.message_bus.dispatcher import default_bus_paths, dispatch_next
        ...
        for attempt in range(4):
            try:
                dispatch_result = dispatch_next(paths=paths, live=True)
                break
                ...
                return (
                    False,
                    f"D2A dispatch blocked: {last_error}",
                    meta,
                )
                ...
                return (
                    False,
                    f"D2A dispatch failed for {agent_id}: {exc}",
                    meta,
                )
        ...
        if dispatch_result and not dispatch_result.success:
            ...
            return (
                False,
                f"D2A dispatch failed for {agent_id}: {dispatch_result.detail}",
                meta,
            )
        meta["dispatch_error"] = last_error or "no queued message after enqueue"
        return (
            False,
            f"D2A enqueued but live dispatch did not run for {agent_id}",
            meta,
        )
    except Exception as exc:
        logger.exception("message bus send failed")
        return False, str(exc), {"message_bus": "error", "error": str(exc)}
