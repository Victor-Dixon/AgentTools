...
Identity:
You are Agent-3. Act as this agent for this message.
If you are not Agent-3, do NOT reply; forward to Agent-3.
...
- Ownership Boundary: modify ONLY `agent_workspaces/Agent-3/**` unless task explicitly assigns other paths
...
────────────────────────────────────────
```bash
cat agent_workspaces/Agent-3/rehydration.json
```
...
────────────────────────────────────────
```bash
python tools/force_multiplier_gate.py --agent Agent-3
```
...
python tools/one_screen_execution_loop.py --agent Agent-3
